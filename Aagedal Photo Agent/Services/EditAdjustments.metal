...
struct EditParams {
    ...
    uint activeFlags;        // bitmask: bit0=exposure, bit1=vibrance,
    ...
    uint2 gid [[thread_position_in_grid]])
{
    ...
    // 1. Exposure: exp2(ev) — exact match to CIExposureAdjust
    if (params.activeFlags & (1u << 0)) {
        rgb *= half3(exp2((half)params.exposure));
    }

    // 2. Vibrance: selective saturation boost on less-saturated pixels
    ...
    // 3. Saturation — exact match to CIColorControls saturation
    if (params.activeFlags & (1u << 2)) {
        half lum = dot(rgb, half3(0.2126h, 0.7152h, 0.0722h));
        rgb = mix(half3(lum), rgb, (half)params.saturation);
    }

    // 4. White balance (3x3 matrix extracted from CITemperatureAndTint)
