...
private final class Renderer {
    ...
    private func drawLocationEvidence() {
        ...
        if let solar = map.solarEvidence {
            cursorY -= 16
            drawSolarEvidence(solar)
        }
        if options.includeLocationCoordinates {
    ...
    private func drawLimitations() {
        ...
        if snapshot.mapEvidence?.solarEvidence != nil {
            limitations.append(contentsOf: [
        ...
        for limitation in limitations {
            ...
            ensureSpace(textHeight + lineSafety + 8)
            drawFlowingText(text, font: font, color: ink, lineSpacing: lineSpacing, leftInset: 10)
            cursorY -= 8
        }
    }
    ...
    private func drawMapSchematic(_ map: AnalysisReportMapEvidence, in rect: CGRect) {
        ...
        if let solar = map.solarEvidence,
           let renderModel = AnalysisSolarMapRenderModel(
               overlay: solar.overlay,
               origin: solar.coordinate,
               latitudeDelta: map.viewport.latitudeDelta,
               longitudeDelta: map.viewport.longitudeDelta,
               day: solar.day
           ) {
            drawSolarRays(renderModel, viewport: map.viewport, rect: rect)
            drawSolarLegend(renderModel, in: rect)
        }
    ...
    private func drawSolarEvidence(_ solar: AnalysisReportSolarEvidence) {
        drawSubheading("Solar position calculation")
        drawKeyValue("Input time", solar.overlay.timestamp.formatted)
        if options.includeLocationCoordinates {
            drawKeyValue(
                "Input coordinate",
                coordinate(
                    latitude: solar.coordinate.latitude,
                    longitude: solar.coordinate.longitude
                )
            )
        } else {
            drawKeyValue("Input coordinate", "Omitted by export settings")
        }
        let place = solar.locationPlaceName.map { " · \($0)" } ?? ""
        drawKeyValue(
            "Coordinate provenance",
            "\(solar.locationSource.displayName)\(place): \(solar.locationSourceDetail)"
        )
        if let linked = solar.linkedTimestampEvidence {
            drawKeyValue(
                "Time provenance",
                "\(linked.title) · \(linked.source.displayName): \(linked.sourceDetail)"
            )
        } else if solar.overlay.linkedTimestampEvidenceID != nil {
            drawKeyValue(
                "Time provenance",
                "Linked timeline evidence unavailable; frozen timestamp retained"
            )
        } else {
            drawKeyValue("Time provenance", "Manual case-only date and fixed UTC offset")
        }
        drawKeyValue("Calculation method", solar.day.method.displayName)
        drawKeyValue("Sun azimuth", solarDegrees(solar.day.position.azimuthDegrees))
        drawKeyValue(
        ...
        drawKeyValue(
            "Apparent elevation",
            solarSignedDegrees(solar.day.position.apparentElevationDegrees)
        )
        drawKeyValue("Sunrise", solarEvent(solar.day.sunrise, solar: solar))
        drawKeyValue("Solar noon", solarEvent(solar.day.solarNoon, solar: solar))
        drawKeyValue("Sunset", solarEvent(solar.day.sunset, solar: solar))
        if let condition = solar.day.polarCondition {
            drawKeyValue("Polar condition", condition == .polarDay ? "Polar day" : "Polar night")
        } else if solar.day.position.isBelowHorizon {
            drawKeyValue("Sun state", "Below the apparent horizon")
        }
        let selectedRays = AnalysisSolarMapRayKind.allCases.compactMap { kind -> String? in
            switch kind {
            case .sun where solar.overlay.showsSunDirection: kind.displayName
            case .shadow where solar.overlay.showsShadowDirection: kind.displayName
            case .sunrise where solar.overlay.showsSunriseDirection: kind.displayName
            case .sunset where solar.overlay.showsSunsetDirection: kind.displayName
            default: nil
            }
        }
    ...
    private func drawSolarRays(
        _ renderModel: AnalysisSolarMapRenderModel,
        viewport: AnalysisMapViewport,
        rect: CGRect
    ) {
        for ray in renderModel.rays {
            let path = NSBezierPath()
            path.move(to: projected(ray.origin, viewport: viewport, rect: rect))
            path.line(to: projected(ray.destination, viewport: viewport, rect: rect))
            let style = solarRayStyle(ray.kind)
            style.color.setStroke()
            path.lineWidth = style.lineWidth
            if !style.dash.isEmpty {
                path.setLineDash(style.dash, count: style.dash.count, phase: 0)
            }
            path.stroke()
        }
    }

    private func drawSolarLegend(
        _ renderModel: AnalysisSolarMapRenderModel,
        in rect: CGRect
    ) {
        guard !renderModel.rays.isEmpty else { return }
        let rowHeight: CGFloat = 14
        let legendWidth: CGFloat = 174
        let legendHeight = CGFloat(renderModel.rays.count) * rowHeight + 12
        let legendRect = CGRect(
            x: rect.minX + 8,
            y: rect.maxY - legendHeight - 8,
            width: legendWidth,
            height: legendHeight
        )
        NSColor.white.withAlphaComponent(0.88).setFill()
        NSBezierPath(roundedRect: legendRect, xRadius: 5, yRadius: 5).fill()
        for (index, ray) in renderModel.rays.enumerated() {
            let y = legendRect.maxY - 11 - CGFloat(index) * rowHeight
            let style = solarRayStyle(ray.kind)
            let sample = NSBezierPath()
            sample.move(to: CGPoint(x: legendRect.minX + 8, y: y))
            sample.line(to: CGPoint(x: legendRect.minX + 30, y: y))
            style.color.setStroke()
            sample.lineWidth = style.lineWidth
            if !style.dash.isEmpty {
                sample.setLineDash(style.dash, count: style.dash.count, phase: 0)
            }
            sample.stroke()
            drawText(
                "\(ray.kind.displayName) \(solarDegrees(ray.bearingDegrees))",
                font: .systemFont(ofSize: 7.5, weight: .medium),
                color: ink,
                in: CGRect(
                    x: legendRect.minX + 36,
                    y: y - 5,
                    width: legendRect.width - 42,
                    height: 10
                )
            )
        }
    ...
    private func solarRayStyle(
        _ kind: AnalysisSolarMapRayKind
    ) -> (color: NSColor, lineWidth: CGFloat, dash: [CGFloat]) {
        switch kind {
        case .sun:
            (NSColor(srgbRed: 0.95, green: 0.72, blue: 0.04, alpha: 1), 3, [])
        case .shadow:
            (NSColor(srgbRed: 0.48, green: 0.24, blue: 0.68, alpha: 1), 2.4, [7, 5])
        case .sunrise:
            (NSColor(srgbRed: 0.90, green: 0.42, blue: 0.10, alpha: 0.82), 1.6, [])
        case .sunset:
    ...
    private func solarSignedDegrees(_ value: Double) -> String {
        String(format: "%+.2f°", locale: Locale(identifier: "en_US_POSIX"), value)
    }

    private func solarEvent(
        _ event: AnalysisSolarEvent?,
        solar: AnalysisReportSolarEvidence
    ) -> String {
        guard let event else { return "Does not occur" }
        let offset = solar.overlay.timestamp.utcOffsetMinutes ?? 0
        let timeZone = TimeZone(secondsFromGMT: offset * 60) ?? .gmt
        let timestamp = AnalysisTimestampValue(
            date: event.instant,
            precision: .minute,
            timeZone: timeZone
        )
        return "\(timestamp.formatted) · \(solarDegrees(event.azimuthDegrees))"
    }
