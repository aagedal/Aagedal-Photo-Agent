...
/// Canonical ISO 3166-1 country identity used by IPTC Country Code.
///
/// Metadata stores the stable alpha-3 code. Localized names are derived only for presentation,
/// so changing the app or system language never changes the value written to XMP/IIM.
nonisolated struct ISO3166Country: Identifiable, Hashable, Sendable {
    let alpha2: String
    let alpha3: String

    var id: String { alpha3 }

    func localizedName(locale: Locale = .current) -> String {
        locale.localizedString(forRegionCode: alpha2) ?? alpha3
    }

    static let all: [Self] = {
        alpha2Alpha3Pairs.split(whereSeparator: \.isWhitespace).compactMap { pair in
            let components = pair.split(separator: ":", maxSplits: 1)
            guard components.count == 2 else { return nil }
            return Self(alpha2: String(components[0]), alpha3: String(components[1]))
        }
    }()

    static let alpha3Codes = Set(all.map(\.alpha3))

    static func normalizedAlpha3(_ value: String?) -> String? {
        guard let value else { return nil }
        let normalized = value.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
        return normalized.isEmpty ? nil : normalized
    }

    static func isValidAlpha3(_ value: String) -> Bool {
        alpha3Codes.contains(value)
    }

    // Current ISO 3166-1 assignments. Kosovo's user-assigned XK/XKX code and the withdrawn AN/ANT
    // and CS/SCG entries are deliberately excluded from standards validation.
    private static let alpha2Alpha3Pairs = """
