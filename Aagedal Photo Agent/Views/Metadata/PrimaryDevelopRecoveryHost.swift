...
/// This presentation lives outside the flush barrier that retained Primary Develop work can block.
struct PrimaryDevelopRecoveryHost: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    Text("Review Retained Develop Edits").font(.title2.bold())
                    if let review {
                        ...
                        Text("Export first to keep a copy of the captured Develop settings, metadata, original file evidence, history and save details. Discard removes only these exported requests from this session. It keeps saved photos, metadata files and retained requests for other photos.")
                        ...
                        ScrollView {
                            VStack(alignment: .leading, spacing: 10) {
                                ForEach(viewModel.primaryDevelopRecoveryPhotos, id: \.self) { photo in
                                    ...
                                    .accessibilityIdentifier("develop.conflictReview.photo.\(photo.path)")
                                }
                            }
                        }
                        .frame(maxHeight: 230)
                        if viewModel.primaryDevelopRecoveryPhotos.isEmpty {
                            Text("No Develop requests remain to review.").foregroundStyle(.secondary)
                        }
                    }
                    if let errorMessage {
                        Label(errorMessage, systemImage: "exclamationmark.triangle")
                            .foregroundStyle(.red).textSelection(.enabled)
                            .accessibilityIdentifier("develop.conflictReview.error")
                    }
                }
                .frame(maxWidth: .infinity, alignment: .leading)
            }
            ...
            HStack {
                ...
                if review != nil {
                    Button("Export Recovery JSON…", action: export)
                        ...
                        .accessibilityIdentifier("develop.conflictReview.export")
                    Button("Discard Exported Requests", role: .destructive, action: discard)
                        .disabled(receipt == nil)
                        .accessibilityIdentifier("develop.conflictReview.discard")
                }
            }
        }
        .padding(24)
        ...
        .accessibilityIdentifier("develop.conflictReview")
        .onAppear {
            lifetime = UUID()
        ...
        .onDisappear {
            ...
            if !discardInFlight, let review { Task { await viewModel.endPrimaryDevelopRecovery(review) } }
        }
    }
    ...
    private func begin(_ photo: URL) {
        ...
        Task { @MainActor in
            do {
                let captured = try await viewModel.beginPrimaryDevelopRecovery(for: photo)
                guard owns(id, expected) else {
                    await viewModel.endPrimaryDevelopRecovery(captured)
                    return
                }
    ...
    private func export() {
        ...
        panel.nameFieldStringValue = "\(review.photoURL.lastPathComponent) Develop Recovery.json"
        panel.title = "Export Retained Develop Edits"
        let completion: (NSApplication.ModalResponse) -> Void = { response in
            Task { @MainActor in
                ...
                do {
                    let exported = try await viewModel.exportPrimaryDevelopRecovery(review, to: url)
                    guard owns(id, expected) else { return }
                    receipt = exported
                } catch {
                    guard owns(id, expected) else { return }
    ...
    private func discard() {
        ...
        Task { @MainActor in
            do {
                try await viewModel.discardPrimaryDevelopRecovery(review, receipt: receipt)
                guard owns(id, expected) else { return }
                self.review = nil; self.receipt = nil; operation = nil; discardInFlight = false
                dismiss()
            } catch {
                guard owns(id, expected) else {
                    await viewModel.endPrimaryDevelopRecovery(review)
                    return
                }
    ...
    private func cancel() {
        ...
        Task { @MainActor in
            if let review { await viewModel.endPrimaryDevelopRecovery(review) }
            guard owns(id, expected) else { return }
