...
- [x] Phase 3 — Compositing kernel: `maskType` branch in `editAdjustments` — **done 2026-07-01**. `MaskParams` gains `uint maskType` (0=ellipse SDF, 1=brush) + `uint brushLayer` (slice index) in both the Metal struct and the Swift mirror (stride 88→96, verified matching). `editAdjustments` takes a new `texture2d_array<half> brushAlpha [[texture(3)]]` argument; the per-mask branch in the order-buffer loop samples `brushAlpha` at source `uv` for `maskType==1` (applying inverted+amount to match the ellipse path) or falls back to `maskWeight()` — `applyMaskColor` unchanged, so exposure/Anonymizer/Temp-Tint all work on brush masks with zero new code. CPU wiring in `MetalEditPipeline`: removed the `$0.brush == nil` filter guard in `updateParams` (both kinds now participate); brush masks encode `maskType=1`/`brushLayer` (assigned in enabled-mask order, matching the alpha slice order); `refreshBrushAlpha(_:size:)` caches the rasterization by `[BrushMaskGeometry] == lastBuilt` + size so the full-res GPU rebuild only fires on stroke/resolution/image change, not per tonal slider drag; a 1×1×1 `emptyBrushAlpha` placeholder is bound whenever there are no brush masks (Metal requires every declared texture bound). Live `render()` and `renderOffscreenSerial` (export) both bind `brushAlphaTexture ?? emptyBrushAlpha` at texture(3); the offscreen path rebuilds explicitly at working resolution since the shared export pipeline has no persistent source texture. **Orientation handled here (not deferred):** added `BrushMaskGeometry.transformedForDisplay/ForSensor(orientation:)` (point-maps each dab's UV exactly like the ellipse center; no aspect term since a dab is a long-edge-relative circle) and wired both into `MaskAdjustment.transformedFor…`. 2 new end-to-end tests in `BrushCompositingTests` (brush exposure brightens only the painted region; ellipse SDF path still renders after the branch) render through `renderOffscreen` and read back pixels; all 12 brush tests + EditExport/Crop/ToneCurve suites (41 tests) green.
- [x] Phase 4 — UI: bare-`B` paint tool, `BrushMaskOverlayNSView`, brush settings toolbar — **done 2026-07-01** (interactive paint verification pending — see below). `BrushMaskOverlayRepresentable` + `BrushMaskOverlayNSView` appended to `MaskOverlayNSView.swift` (sibling of the ellipse overlay, same target member — no pbxproj change): captures mouseDown/Dragged/Up, maps screen→display-UV with the identical `viewportOrigin/Size` projection the ellipse overlay uses, spacing-filters dabs to 25% of brush diameter (with interpolation so fast drags stay continuous), draws a brush-size cursor ring, and streams incremental dabs via callbacks. Wired into `EditWorkspaceView`: bare-`B` toggles `isBrushPainting` (added after the bare-`J` handler; deselects the WB eyedropper); a `brushOverlay(viewportOrigin:viewportSize:viewSize:)` helper is placed in BOTH the normal-fit (Region C) and crop-applied (Region B) preview layouts, above the ellipse/WB overlays; the ellipse overlay is now gated `brush == nil && !isBrushPainting` so it doesn't fight for the mouse or draw handles on a brush mask. Callbacks: `onStrokeBegan`→`ensureBrushTarget` (selects the current brush mask or lazily creates one via `addNewBrushMask`, runs `updateParams` so the new mask's alpha slice is allocated, returns the slice index computed by `brushSliceIndex` mirroring the pipeline's enabled-mask slice assignment); `onStrokeChanged`→`liveBrushStamp` (calls `pipeline.stampBrushStroke` incrementally + `metalCoordinator.requestRedraw()` for immediate feedback, no model mutation); `onStrokeEnded`→`commitBrushStroke` (converts the display-frame stroke to sensor frame via `brushStrokeForSensor`, appends it to the mask's `brush.strokes` in ONE `updateCameraRaw` = one undo entry, rebuilds the authoritative alpha, `commitEditAdjustments`). Transient `brushToolbar` (Size/Hardness/Flow sliders + Add/Erase segmented control) shown in the mask panel only while painting. Pipeline gained a `stampBrushStroke(_:layer:)` incremental live-stamp entry (shares `encodeStroke` with `rebuildBrushAlpha`) so dragging doesn't do a full-res rebuild per tick. 3 new tests (live-stamp accumulate/guard rasterization tests + a display↔sensor brush orientation round-trip) added; 18 brush tests total + EditExport/Sidecar (28) green; app builds and launches. **Not yet verified: actually painting in the running app** (computer-use lacked Accessibility/Screen-Recording grants this session) — flagged as the manual check.
- [x] Phase 5 — Undo, layer-strip icon, paste-to-other-images plumbing — **done 2026-07-01** (mostly satisfied by Phase 4). Per-gesture undo: one `updateCameraRaw` per stroke = one undo entry (from Phase 4's `commitBrushStroke`). Layer-strip icon: `paintbrush.pointed` renders automatically via `MaskAdjustment.layerKind` → `.brushMask` (no extra work). Paste-to-other-images: **user-verified working** — brush strokes ride along in `localAdjustments` through the existing mask-array copy/paste, no special-casing needed. Additional Phase-4-era UX shipped this session: two stacked one-click add-mask buttons (radial/brush) replacing a menu; bare-`X` toggles Add/Erase; ACR-style red mask-coverage overlay (kernel `maskOverlayIndex`, auto-hides once the mask is adjusted); brush toolbar persists for a selected brush mask (not only while painting) with a Paint toggle; paint mode exits when selection leaves brush layers; soft-eraser blend made symmetric with the add brush (`min(prev, 1-coverage)` envelope). **Minor known gaps (not blockers, deferred):** (1) first stroke on a NEW brush mask = 2 undo entries (create-mask + stroke); (2) crop-STRAIGHTEN angle not applied to brush dabs (only EXIF orientation) — misaligns on a straightened image; (3) no cross-stroke flow build-up (each stroke caps at its flow, unlike ACR accumulation).
- [~] Phase 6 — Calibration pass against real ACR sample files — **deprioritized 2026-07-01 (user decision: ACR interop for brush masks isn't important).** The two unconfirmed constants (`crs:Radius` long-edge normalization reference; `MaskValue`/density semantics) only matter for matching Adobe's exact brush *scale*; the brush is internally self-consistent (paints/erases/saves/reloads/pastes correctly through our own reader), so this is now optional. NOTE: we still write the real ACR `Dabs` format (it round-trips through our own parser at zero extra cost — no reason to switch to app-private), and the Phase-1 "preserve unparseable correction" fallback stays REGARDLESS (it prevents deleting Adobe-authored erase-brush masks opened in our app — a data-loss guard independent of our own interop).
