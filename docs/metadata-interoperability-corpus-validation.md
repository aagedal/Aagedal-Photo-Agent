...
- `legacy-boundaries.json` is a CC0, deterministic recipe corpus covering the UTF-8 byte ceiling
  for all 13 IPTC-IIM text datasets currently dual-written by Photo Agent.
...
- A stable built-in `IPTC-IIM Compatibility` profile exposes warning rules for the 13 editable
...
Result: 19 tests across the two suites passed with no failures. This includes 13 field-boundary
recipes, seven timestamp variants, the portable profile contract, semantic rejection, XMP
preservation, generated embedded-JPEG preservation, and XMP/IIM conflict precedence.
