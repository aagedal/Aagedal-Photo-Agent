...
**Latest improvement continuation:** [Cycle 33: voice-memo accessibility and offline prerequisite](release/cycle-33-voice-memo-accessibility-offline-prerequisite-2026-09-14.md). The transcript preview now exports stable summary, photo and field semantics, receives accessibility focus, and posts privacy-safe typed preview/refusal/cancel/success/failure announcements. A single-launch native accessibility test passes. A new production-recognizer/relaunch drill reaches the explicit language-download prerequisite and skips because English on-device speech is not installed; no implicit system download was attempted. The 2,907-test serial suite and repository checks pass. Spoken VoiceOver observation, completed offline recognition, authorized-Sony, real-server and wider release gates remain open.
...
| Journalistic metadata workflow and local AI automation | [Implementation plan](journalistic-metadata-workflow-plan.md) | Caption, unified metadata-field customization, Batch Rename, Deadline preflight/delivery and information hierarchy, typed editorial metadata, schema migration, preservation, verification, and recovery are implemented; Headline and every localized Title alternative round-trip independently; ILCE-1 v4.00 one/two-source RAW/JPEG voice-memo ingest, persisted relationships, transactional rename, independent Duplicate, companion-aware Move/Reject, recoverable memo Trash, RAW archive preservation, explicit Caption playback/recovery, exact moved-relationship reassociation, Apple on-device editable transcription, exact-WAV-bound reviewed approval persistence, the compatible-target, exact-previewed `{voiceMemoTranscript}` variable, and explicit include/exclude/required WAV delivery with verified receipts are implemented. The newly approved 3.0 scope adds an opt-in local STDIO MCP server plus Aagedal Media Converter's FFmpeg build with embedded whisper.cpp as a second transcription provider; llama.cpp/GGUF is deferred to 3.1. | Implement and verify the MCP/Whisper phase, then complete native Sony/archive/reassociation/transcription/application/delivery evidence, broader Sony samples, external interoperability, real-server, device, accessibility, and first-use Deadline drills |
...
2. Close journalistic workflow release hardening. Caption Workspace, Batch Rename, Deadline Mode,
   ...
   recovery boundaries; remaining work is the final support-ledger model decisions and native/
   authentic validation beyond the implemented ILCE-1 v4.00 ingest, persisted relationships,
   lifecycle operations, playback, local editable transcription, reviewed exact-WAV approval,
   compatible transcript-variable application and verified WAV delivery. Then implement the signed,
   opt-in STDIO MCP facade and FFmpeg Whisper provider using those same guarded operations, plus broader real-sample compatibility,
