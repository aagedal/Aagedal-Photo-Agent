...
[cycle 20](cycle-20-voice-memo-raw-archive-2026-09-12.md). The adjacent missing-memo recovery slice
...
## Source reassociation evidence boundary and implemented recovery slice
...
Schema-1 records contain only version, profile ID, image filename and memo filename. They cannot
prove that a separately found image or WAV is the original bytes. Cycle 21 adds schema 2 with
optional SHA-256/byte-count identities for both photo and WAV, explicit provenance, and an optional
audio-hash binding reserved for reviewed transcript approval. Newly saved, imported, copied, moved
and archived associations capture the available final identities; schema-1 decode remains supported.
...
Cycle 21 implements an explicit Caption recovery action for a relationship whose photo and hidden
record are still present but whose adjacent WAV is missing, with two separate provenances:

- For a previously captured photo and memo revision, discover only within user-selected
  candidate files. Exact photo and WAV bytes may reattach automatically.
...
The implementation preserves unknown JSON fields, copies rather than moves a selected WAV, stages
and verifies it before exclusive installation, revalidates photo/candidate/record evidence, restores
the exact prior record and removes only its owned copy on failure, refreshes playback without
autoplay, and prevents cancelled or superseded Caption sessions from publishing a stale result.
