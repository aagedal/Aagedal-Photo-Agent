...
**Updated:** 2026-09-15
**Latest implementation state:** Cycle 41 adds a shared explicit-photo read facade and a read-only revision-evidence tool for source, adjacent XMP and owned app JSON. Its 14-test focused MCP suite, direct built-helper discovery, repaired 17-execution RAW archive cancellation suite, combined-source complete serial suite (Xcode result: 3,767 executions, zero failures and zero skips) and repository validation pass. Cycle 40 shares Browser input-format admission with the MCP helper, exposes bounded read-only format discovery and tightens malformed/unknown tool-call handling; the 2,922-test complete serial suite and repository validation pass. Cycle 39 extends the shared folder lease over Batch Rename execution and post-move reassociation; 19 focused tests, the 2,920-test complete serial suite and repository validation pass. Cycle 38 extends the shared photo lease over variable/template JSON preparation, physical completion and semantic read-back; 37 focused tests, the 2,919-test complete serial suite and repository validation pass. Cycle 37 adds copyable Codex CLI, Claude Code and both OpenCode setup formats; live client connection remains unverified. Cycle 36 added cross-process reservations around retained field mutation, Write All and Primary Develop saves. Production workflow tools, broader operation coordination, FFmpeg Whisper, independent review and native-client evidence remain open. Cycle 31's 10-test pass remains the latest complete UI-smoke-target result because later whole-target attempts encountered desktop/menu focus instability.
...
5. Continue the local transcription slice following the [SDK-backed design](voice-memo-transcription-design.md).
   ...
   delivery.
6. Continue the mandatory [Phase 5A MCP/Whisper scope](../journalistic-metadata-workflow-plan.md#phase-5a--local-mcp-automation-and-ffmpeg-whisper-transcription). Cycles 34–39 establish the bundled default-off STDIO helper, root admission, client setup and shared reservations for several retained GUI writes and Batch Rename execution. Cycle 40 adds input-format discovery and strict tool-call shapes; cycle 41 starts the shared explicit-photo read facade with carrier revision tokens. Next anchor carrier traversal to the authorized-root descriptor and cover ancestor retargeting, then connect the production metadata reader for typed descriptive values, source/carrier and pending/conflict state. Connect remaining GUI operations to shared admission; add operation status/cancellation, face scan, metadata/Develop template and batch transcription tools, two-phase IPTC patch commits, and the Aagedal Media Converter FFmpeg build with embedded whisper.cpp plus hardened model delivery. Verify launch/discovery from each targeted client using disposable configuration. Preserve Apple Speech and the existing review/approval boundary. llama.cpp and general GGUF inference remain 3.1.
...
| Automated regression and package | Cycle 41: 14 focused MCP tests, 17 RAW archive tests and final-source complete serial suite pass (Xcode result: 3,767 executions, zero failures, zero skips); cycle 31 retains the latest complete 10-test UI run | Complete UI target, packaging and exact-candidate release checks |
