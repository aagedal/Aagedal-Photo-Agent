...
**State:** IMPLEMENTING — Known People interchange/cloud reconciliation, transactional voice-memo archive/recovery/reassociation, local transcription, durable reviewed approval, exact-approved transcript-variable resolution, accessible pre-mutation application preview, native single- and multi-image application/read-back with invalid-authority refusal, and explicit verified WAV delivery pass verification. Mandatory 3.0 MCP work now has a bundled hardened-runtime STDIO transport, default-off Settings/root authority, read-only tool annotations, strict path admission and a shared photo/folder reservation primitive used by three retained GUI write flows. The shared production-service facade, complete GUI/MCP operation admission/status/cancellation, guarded IPTC mutation tools and Aagedal Media Converter FFmpeg/Whisper provider remain. llama.cpp/GGUF is explicitly deferred to 3.1. Installed-language offline completion, spoken VoiceOver, authentic Sony/real-server voice-memo evidence, cloud and broader release gates also remain.
**Updated:** 2026-09-15
**Latest implementation state:** Cycle 37 adds copyable Codex CLI, Claude Code, OpenCode 1.x and OpenCode v2 setup in Automation Settings. The app builds, and its bundled helper completes an OpenCode-shaped STDIO initialize/tool-discovery smoke check. Live client connection remains unverified. Cycle 36 added the shared cross-process reservation primitive around retained field mutation, Write All and Primary Develop saves; its app build and second-process smoke pass, while Xcode test assertions are pending after compiler-driver interruptions. Cycle 35's SwiftMediaMetadata 3.0.1 refresh remains the latest complete 2,916-test serial suite. Cycle 34 supplies the MCP transport, root authority and three read-only tools. Production workflow tools, broader operation coordination, FFmpeg Whisper, independent review and native-client evidence remain open. Cycle 31's 10-test pass remains the latest complete UI-smoke-target result because later whole-target attempts encountered desktop/menu focus instability.
...
classifications and concrete next actions. Current authoritative plans contain 85 unchecked criteria
(9 audit, 23 investigation, 47 journalistic, 6 solar) after the newly approved MCP/Whisper phase was
...
| Automated regression and package | Cycle 35: 2,916 serial tests passed before cycle 36; cycle 36 app build and second-process smoke pass, with Xcode test assertions pending; cycle 31 retains the latest complete 10-test UI run | Rerun focused/full tests on current source, complete UI target, packaging and exact-candidate release checks |
