...
**State:** IMPLEMENTING — Known People interchange/cloud reconciliation, transactional voice-memo archive/recovery/reassociation, local transcription, durable reviewed approval, exact-approved transcript-variable resolution, accessible pre-mutation application preview, native single- and multi-image application/read-back with invalid-authority refusal, and explicit verified WAV delivery pass verification. Mandatory 3.0 MCP work now has a bundled hardened-runtime STDIO transport, default-off Settings/root authority, read-only tool annotations, strict path admission and a shared photo/folder reservation primitive used by retained field, variable/template, Write All and Primary Develop GUI write flows. The shared production-service facade, complete GUI/MCP operation admission/status/cancellation, guarded IPTC mutation tools and Aagedal Media Converter FFmpeg/Whisper provider remain. llama.cpp/GGUF is explicitly deferred to 3.1. Installed-language offline completion, spoken VoiceOver, authentic Sony/real-server voice-memo evidence, cloud and broader release gates also remain.
**Updated:** 2026-09-15
**Latest implementation state:** Cycle 38 extends the shared photo lease over variable/template JSON preparation, physical completion and semantic read-back; 37 focused tests, the 2,919-test complete serial suite and repository validation pass. Cycle 37 adds copyable Codex CLI, Claude Code and both OpenCode setup formats; live client connection remains unverified. Cycle 36 added cross-process reservations around retained field mutation, Write All and Primary Develop saves. Cycle 34 supplies the MCP transport, root authority and three read-only tools. Production workflow tools, broader operation coordination, FFmpeg Whisper, independent review and native-client evidence remain open. Cycle 31's 10-test pass remains the latest complete UI-smoke-target result because later whole-target attempts encountered desktop/menu focus instability.
