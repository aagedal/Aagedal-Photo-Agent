...
**Updated:** 2026-09-15
**Latest implementation state:** Cycle 45 compares carrier change time and path-entry identity during anchored reads, and bounds oversized STDIO errors without allocating dummy request buffers. Its final-source complete serial suite passes 3,777 executions, zero failures and zero skips; repository validation passes. Cycle 44 exposes bounded basic descriptive fields from the exact-owner app JSON draft while clearly marking them unreconciled. Cycles 41–43 established root-anchored source/XMP/app-JSON revision evidence and draft state; cycles 34–40 established bundled default-off transport, client setup, shared reservations, format discovery and strict tool-call shapes. Effective typed metadata reads, production workflow tools, broader operation coordination, FFmpeg Whisper, independent review and native-client evidence remain open. Cycle 31's 10-test pass remains the latest complete UI-smoke-target result because later whole-target attempts encountered desktop/menu focus instability.
...
**Cycle baseline:** `c69575f` on `main`; cycles 16–33 advance Known People and Sony voice-memo workflows, cycles 34–40 establish MCP transport, authority, reservations and discovery, cycles 41–43 add anchored revision/draft-state reads, cycle 44 exposes bounded app-owned descriptive draft fields, and cycle 45 tightens carrier stability and STDIO bounds.
