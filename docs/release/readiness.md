...
**Updated:** 2026-09-19
**Latest implementation state:** Cycle 49 shares the production effective-metadata resolver between editor reference selection and Copy Previous, retaining pending-draft/physical-state rules and exposing descriptive provenance and conflict state. Copy Previous now refuses incomplete XMP evidence. Focused validation passes 66 tests / 108 parameterized executions with no failures/skips; the complete serial suite passes 2,969 tests / 3,821 parameterized executions with no failures/skips, and repository validation passes. Cycle 48's bounded MCP carrier reads remain in place. Effective typed MCP reads, production workflow tools, broader operation coordination, FFmpeg Whisper, independent review and native-client evidence remain open.
...
**Cycle baseline:** `8ca3f1d` for cycle 49; checkout initially clean. This cycle changes shared metadata resolution and its editor/Copy Previous callers, regression tests and release documentation. No MCP endpoint or release-readiness gate is claimed complete.
