...
**Updated:** 2026-09-19
**Latest implementation state:** Cycle 48 fixes FIFO sidecars blocking the MCP helper before regular-file admission and bounds JSON capture plus revision hashing to the captured size and one growth probe. The 25-test focused MCP suite, complete 2,956-test serial suite and repository validation pass with zero failures/skips. Cycle 47's retained ancestor checks and cycles 44–46's bounded draft and coherent-generation checks remain in place. Effective typed metadata reads, production workflow tools, broader operation coordination, FFmpeg Whisper, independent review and native-client evidence remain open. Cycle 31's 10-test pass remains the latest complete UI-smoke-target result because later whole-target attempts encountered desktop/menu focus instability.
...
**Cycle baseline:** `45d77c1` for cycle 48, including intervening committed variable handling and face-name UI changes. The checkout was clean before the two MCP Swift edits and release documentation updates. Cycle 48 adds bounded carrier reads to the transport, authority, shared reservations, discovery and revision/draft-evidence work from cycles 34–47.
