...
**Updated:** 2026-09-15
**Latest implementation state:** Cycle 34 MCP transport and authority adds the bundled `photo-agent-mcp` target, coherent cross-process enablement/root preferences, exact-root identity checks, strict existing-target admission, protocol lifecycle and three read-only tools. Nine focused tests, the exact bundled helper launch/signature check, the 2,916-test serial suite and repository validation pass. Production workflow tools, operation coordination, FFmpeg Whisper, independent review and native-client evidence remain open. Cycle 31's 10-test pass remains the latest complete UI-smoke-target result because later whole-target attempts encountered desktop/menu focus instability.
