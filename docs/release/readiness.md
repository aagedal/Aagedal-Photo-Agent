...
**Updated:** 2026-09-15
**Latest implementation state:** Cycle 46 rechecks all source, XMP and current/legacy app-JSON path entries before returning combined revision and draft evidence. Its final-source complete serial suite passes 3,778 tests, zero failures and zero skips; repository validation passes. Cycle 45 tightened each anchored read and STDIO bounds, and cycle 44 exposed bounded basic descriptive fields from the exact-owner app JSON draft while clearly marking them unreconciled. Cycles 41–43 established root-anchored source/XMP/app-JSON revision evidence and draft state; cycles 34–40 established bundled default-off transport, client setup, shared reservations, format discovery and strict tool-call shapes. Effective typed metadata reads, production workflow tools, broader operation coordination, FFmpeg Whisper, independent review and native-client evidence remain open. Cycle 31's 10-test pass remains the latest complete UI-smoke-target result because later whole-target attempts encountered desktop/menu focus instability.
...
**Cycle baseline:** `0ab8af7` on `main`; cycles 16–33 advance Known People and Sony voice-memo workflows, cycles 34–40 establish MCP transport, authority, reservations and discovery, cycles 41–43 add anchored revision/draft-state reads, cycle 44 exposes bounded app-owned descriptive draft fields, cycle 45 tightens carrier stability and STDIO bounds, and cycle 46 refuses mixed-generation carrier evidence.
...
5. Continue the local transcription slice following the [SDK-backed design](voice-memo-transcription-design.md).
   ...
   delivery.
6. Continue the mandatory [Phase 5A MCP/Whisper scope](../journalistic-metadata-workflow-plan.md#phase-5a--local-mcp-automation-and-ffmpeg-whisper-transcription). Cycles 34–39 establish the bundled default-off STDIO helper, root admission, client setup and shared reservations for several retained GUI writes and Batch Rename execution. Cycles 40–46 add input-format discovery, strict tool-call shapes, root-anchored photo/carrier revisions, bounded owned-sidecar draft fields and final mixed-generation refusal. Next connect the production metadata reader for typed descriptive values, effective source/carrier and conflict state; cover ancestor retargeting under concurrent filesystem change. Connect remaining GUI operations to shared admission; add operation status/cancellation, face scan, metadata/Develop template and batch transcription tools, two-phase IPTC patch commits, and the Aagedal Media Converter FFmpeg build with embedded whisper.cpp plus hardened model delivery. Verify launch/discovery from each targeted client using disposable configuration. Preserve Apple Speech and the existing review/approval boundary. llama.cpp and general GGUF inference remain 3.1.
