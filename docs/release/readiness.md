...
**Updated:** 2026-09-19
**Latest implementation state:** Cycle 66 extends the reserved face deletion transaction to individual face IDs, preserving newer disk faces/group names and same-photo peers. Busy admission and failed saves retain visible results; consecutive deletions serialize and stale local edits cannot restore removed faces. Focused validation passes 25 tests; the full suite passes 3,017 tests across 319 suites with zero failures or skips. Repository validation passes. The same four previously recorded QoS runtime warnings remain. General cross-process stale-save reconciliation, the separate group/photo-trash workflow, other face mutation routes, production MCP workflows and FFmpeg Whisper remain open.
...
**Cycle baseline:** `5690801` for cycle 66; checkout initially clean. Working-tree changes contain individual face deletion reconciliation and regression coverage. No broad release-readiness gate is claimed complete.
