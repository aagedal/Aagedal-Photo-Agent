...
**Updated:** 2026-09-15
**Latest implementation state:** Cycle 43 adds bounded owned-sidecar draft-state inspection, duplicate-owner refusal and carrier absence/directory identity rechecks on the cycle-42 root-anchored photo read. The combined final-source serial suite passes 3,769 executions, zero failures and zero skips; repository validation passes. Cycle 42 anchors photo revision reads to the granted root through no-follow directory descriptors, including nested carriers and private-folder link refusal. Cycle 41 adds the shared explicit-photo read facade and read-only source/XMP/app-JSON revision evidence; its direct built-helper discovery and repaired RAW archive cancellation evidence remain valid. Cycles 34–40 establish the bundled default-off STDIO transport, client setup, shared reservations, format discovery and strict tool-call shapes. Production workflow tools, broader operation coordination, FFmpeg Whisper, independent review and native-client evidence remain open. Cycle 31's 10-test pass remains the latest complete UI-smoke-target result because later whole-target attempts encountered desktop/menu focus instability.
...
**Cycle baseline:** `78f0209` on `main`; cycles 16–19 advance Known People interchange, cycle 20 adds transactional RAW voice-memo archive preservation, cycle 21 adds identity-bound adjacent memo recovery, cycle 22 adds exact moved-relationship reassociation, cycle 23 adds local editable transcription, cycle 24 adds durable reviewed approval, cycle 25 adds the exact-approved shared transcript variable, cycle 26 adds compatible-target enforcement plus its pre-mutation preview, cycle 27 adds explicit verified WAV delivery, cycle 28 adds the transcription failure/reservation matrix, cycle 29 makes incremental review edits durable through navigation and relaunch boundaries, cycle 30 closes the native review/relaunch/approval check, cycle 31 closes the narrow native variable application/read-back check, cycle 32 closes native invalid-authority refusal plus two-photo application, cycle 33 adds preview accessibility semantics, typed announcements and the production offline-recognition drill, cycle 34 starts Phase 5A with the signed STDIO transport and authorization boundary, cycle 35 refreshes the metadata dependency to SwiftMediaMetadata 3.0.1, cycle 36 begins shared cross-process admission for retained GUI writes, cycle 37 adds client-specific MCP setup for Codex, Claude Code and OpenCode, cycle 38 extends that admission over variable/template completion, cycle 39 reserves Batch Rename execution, cycle 40 adds photo-format discovery and strict tool-call shapes, cycle 41 adds explicit-photo revision evidence inspection, cycle 42 anchors its carrier reads to the granted root, and cycle 43 reports owned-sidecar draft state while refusing ambiguous carriers.
