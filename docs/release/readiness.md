...
**Updated:** 2026-09-20
**Latest implementation state:** Cycle 76 adds exposed read-only `prepare_iptc_patch` previews with exact revisions, bounded typed operations, shared IIM warnings and content-bound expiry; secure bounded canonical Whisper output-file reading; and a pinned FFmpeg source patch with sanitizer fault tests. Full regression passes 3,106 tests / 328 suites, zero failures/skips; focused selection passes 64 tests; repository and embedded-helper protocol checks pass. Source is committed as `acc32e8` (FFmpeg patch) and `2793597` (Swift/help); a complete rebuilt artifact, provider integration and verified IPTC commits remain open. No release-readiness gate is newly closed.
...
**Cycle baseline:** `f0b22f8`, initially clean. Cycle 76 implements IPTC previews, canonical output admission and the reproducible FFmpeg correction; source commits are recorded in the dated cycle report.
...
| Automated regression and package | Cycle 76 final-source complete serial suite passes 3,106 tests / 328 suites, zero failures/skips; repository validation and focused source-patch sanitizers pass. Cycle 74 remains the latest native evidence: 14 distinct passing checks and one skipped opt-in speech case | Complete UI target, packaging and exact-candidate release checks |
